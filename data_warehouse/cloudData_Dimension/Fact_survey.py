...
def fill_Fact_table():
    ...
    try:
        ...
        config_dwh = load_config_gcloud()

        # Establish connections
        conn_op = psycopg2.connect(**config_op)
        conn_gcloud_dwh = psycopg2.connect(**config_dwh)

        # Create cursors
        cursor_op = conn_op.cursor()
        cursor_dwh = conn_gcloud_dwh.cursor()
    ...
    """
        ...
        if 'conn_op' in locals():
            conn_op.close()
        if 'conn_gcloud_dwh' in locals():
            conn_gcloud_dwh.close()
